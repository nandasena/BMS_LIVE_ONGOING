...
@Component({
  selector: 'invoice-quotation',
  templateUrl: './invoice-quotation.component.html',
  styleUrls: ['./invoice-quotation.component.scss']
})
export class InvoiceQuotationComponent implements OnInit {
  ...
  constructor(private invoiceService: InvoiceService, private alertify: AlertifyService, private spinner: NgxSpinnerService, private el: ElementRef, private modalService: NgbModal,private quotationService:QuotationService) { }
  ...
  saveInvoice() {
    if (this.itemToSave.length != 0) {
      ...
      this.alertify.confirm('Create Invoice', 'Are you sure you want to create invoice', function () {
        ...
        innerThis.quotationService.saveQuotation(invoiceTosave).then((response) => {
